...
class Timezones : Plugin() {
    ...
    override fun start(context: Context) {
        ...
        // patch message timestamp
        val timestampField = WidgetChatListAdapterItemMessage::class.java.getDeclaredField(
            "itemTimestamp"
        ).apply { isAccessible = true }

        patcher.after<WidgetChatListAdapterItemMessage>("configureItemTag", Message::class.java) {
            val msg = it.args[0] as Message
            val timestamp = (timestampField.get(this) as TextView).parent as ViewGroup
            val user = CoreUser(msg.author)
            TextView(timestamp.context, null, 0, R.i.UiKit_TextView).apply {
                id = tzId
                typeface =
                    ResourcesCompat.getFont(Utils.appContext, Constants.Fonts.whitney_medium)
                setMessageTime(user, settings.getBool("24hourTime", false), this)
                timestamp.addView(this)
            }
        }
    }
    ...
    private fun getTimezone(id: Long): String? {
        ...
        if (usersList.containsKey(id)) return usersList[id] else {
            ...
            } catch (e: Exception) { // OH MY GOD NOOOO AN EXCEPTION I SHOULD IMMEDIALTLY HANDLE IT IN RIGHT WAY lol mantika moment
            }
        }
        return null
    }
    ...
    @SuppressLint("SetTextI18n")
    @RequiresApi(Build.VERSION_CODES.O)
    private fun setMessageTime(user: User, use24Hour: Boolean, tzView: TextView) {
        Utils.threadPool.execute {
            val timezone = getTimezone(user.id)
            if (timezone != null) {
                Utils.mainThread.post {
